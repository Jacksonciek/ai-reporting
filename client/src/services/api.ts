...
const BASE_URL = process.env.NEXT_PUBLIC_API_URL; // Adjust as needed
...
class ApiService {
    ...
    async getRooms(params: GetRoomsParams = {}): Promise<RoomsResponse> {
        ...
        const url = `${BASE_URL}/api/${USER_ID}/rooms?limit=${limit}&offset=${offset}`;
    ...
    async getMessages(roomId: string): Promise<MessagesResponse> {
        try {
            ...
            const url = `${BASE_URL}/api/rooms/${roomId}/messages`;
    ...
    async sendMessageToBot(roomId: string, message: string): Promise<MessagePairResponse> {
        ...
        try {
            ...
            return {
                user: { text: message },
                bot: {
                    text: response.bot?.text || response.response,
                    image: response.bot?.image,
                    document: response.bot?.document,
                }
            };
        } catch (error) {
